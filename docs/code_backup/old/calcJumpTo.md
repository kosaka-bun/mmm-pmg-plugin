此方法原位于FingeringCalculator.calcIncreaseSeq方法的for (var i = 0; i < seq.notes.Count; i++)循环内

```csharp
//若从前一个音到当前音需要跳跃到达，调用此方法计算当前音的指法和演奏类型
void calcJumpTo() {
                ...
                //大于或等于复音的最高音，用4指或5指
                    ...
                    //若为黑键
                        ...
                        note.finger = 
                            ...
                            note.getNoteDistance(nextChord.minPitch) > 7 ? 
                                5 : 4;
                    }
                }
                //介于复音的最高音与最低音之间
    ...
    //若为黑键
        ...
        //判断是否是三连黑键，若是，此键安排1指
        //若访问第三个键时越界，则不符合
        else if (getNote(i + 2) == null) {
            note.finger = 2;
        } else if (nextNote.isBlackKey &&
            getNote(i + 2).isBlackKey) {
            note.finger = 1;
        }
        //不是三连黑键
        //后一个音是黑键，再后一个音是白键，且再后一个音距离后一个音大于或等于大三度，用1指
        else if (nextNote.isBlackKey && !getNote(i + 2).isBlackKey &&
            nextNote.getNoteDistance(getNote(i + 2)) >= 4) {
            note.finger = 1;
        }
        //通常情况下，用2指
